...
const packageRoot = join(workspace, 'packages', 'dsh-sms')
...
`)
  ...
  writeFileSync(join(workspace, 'tsconfig.host.json'), `${JSON.stringify({
    ...
    files: [],
    references: [
      { path: './packages/dsh-sms/tsconfig.json' },
  ...
  const artifacts = generator.generate(['dsh-sms'], ['host'])
  const artifact = artifacts.find(candidate => candidate.package === 'dsh-sms' && candidate.face === 'host')
  if (artifact === undefined) {
    ...
    throw new Error(`Typert did not emit dsh-sms host exports: ${JSON.stringify({
      discovered,
      artifacts,
