...
const temporary = await mkdtemp(join(tmpdir(), 'dsh-sms-profile-'))
...
try {
  ...
  if (typeof manifest.dependencies?.['dsh-sms'] !== 'string') {
    throw new Error('the disposable web profile did not install dsh-sms')
  }
  if (!manifest.dsh?.profile?.bundles?.includes('dsh-sms')) {
