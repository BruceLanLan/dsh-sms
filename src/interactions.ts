...
/** Host callbacks that bind interaction requests to one correlated SMS turn. */
export interface InteractionOwnership {
  ...
  channelFor(agent: Agent): SmsInboundMessage | undefined
  /** Whether delivery is currently healthy enough to claim an interaction. */
  deliveryHealthy(): boolean
}

/** Correlates DSH approvals/questions with direct SMS response commands. */
export class InteractionBroker {
  ...
  cancelAgent(agent: Agent): void {
    ...
    for (const pending of [...this.questions.values()]) {
      if (pending.agent !== agent) continue
      pending.reject(new Error('The SMS interaction was cancelled.'))
      pending.finish()
    }
  }
  ...
  private async requestQuestions(
    ...
    if (!this.ownership.deliveryHealthy() || channel === undefined) {
      throw new Error('SMS delivery is unavailable; the question was not redirected.')
    }
    if (signal.aborted) throw new Error('The SMS question was cancelled.')
    ...
    const result = new Promise<AskUserQuestionAnswer>((resolve, reject) => {
      ...
      const abort = () => {
        reject(new Error('The SMS question was cancelled.'))
        finish()
      }
      const timer = setTimeout(() => {
        reject(new Error('The SMS question timed out.'))
        finish()
      }, this.timeoutMs)
    ...
    void Promise.resolve().then(() => channel.send(formatQuestions(id, questions))).catch(() => {
      pending?.reject(new Error('The SMS question could not be delivered.'))
      pending?.finish()
    })
    return result
  }
