...
import type { SmsInboundMessage } from './gmessages-runtime.js'
...
/** Routes accepted SMS text into exact DSH turns and root sessions. */
export class SessionRouter {
  ...
  async receive(message: SmsInboundMessage): Promise<void> {
  ...
  async close(): Promise<void> {
    ...
    for (const binding of this.bindings.values()) {
      const queued = this.turns.cancelUnclaimed(
        binding.agent,
        new Error('The SMS router stopped before this prompt ran.'),
      )
  ...
  async reset(): Promise<void> {
    ...
    if (id !== undefined) {
      ...
      if (agent !== undefined) {
        ...
        const queued = this.turns.cancelUnclaimed(
          agent,
          new PluginError('runtime-failed', 'Local SMS routing was disconnected before this prompt ran.'),
        )
  ...
  private async processPrompt(message: SmsInboundMessage, prompt: string): Promise<void> {
  ...
  private async runCommand(message: SmsInboundMessage, command: ParsedCommand): Promise<void> {
    try {
      let response: string
      switch (command.name) {
        case 'help':
  ...
  private async statusText(): Promise<string> {
    const id = this.active.get()
    if (id === undefined) return 'SMS routing is ready. No DSH session is selected; your next prompt will create one.'
    const agent = this.ctx.agents.get(SessionId(id))
  ...
  private async assertSwitchAllowed(): Promise<void> {
    if (this.pendingPromptCount > 0) {
      throw new PluginError('busy', 'An SMS prompt is queued or running. Send /stop before switching.')
    }
    const id = this.active.get()
  ...
  private async ensureActiveAgent(): Promise<Agent> {
    ...
    try {
      ...
      // the benign ownership race as a failed SMS prompt.
  ...
  private async createAgent(): Promise<AgentHandle> {
    const id = SessionId(`sms-${randomUUID()}`)
  ...
  private async send(channel: SmsInboundMessage, text: string): Promise<void> {
...
function assistantText(content: readonly unknown[]): string {
  ...
  // SMS is text-only: deliver the answer as plain text instead of raw
