...
export class TurnCorrelation {
  ...
  begin(agent: Agent, messageId: string, channel: SmsInboundMessage): Promise<string | undefined> {
  ...
  discard(agent: Agent, messageId: string): void {
    this.failUnclaimed(
      agent,
      messageId,
      new PluginError('runtime-failed', 'DSH discarded the queued SMS prompt.'),
    )
  }
  ...
  /** Whether this Agent's currently correlated turn belongs to SMS. */
  ...
  close(): void {
    ...
    for (const wait of waits) {
      ...
      wait.reject(new Error('The SMS router stopped.'))
    }
  }
}
